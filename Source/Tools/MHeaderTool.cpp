...
void WriteGeneratedSource(std::ofstream& Out, const SParsedClass& ParsedClass)
{
    ...
    {
        Out << "\n// ============================================\n";
        Out << "// Injection: " << ParsedClass.Name << " -> " << ParsedClass.InjectionClass << "\n";
        Out << "// Generated by MHeaderTool\n";
        Out << "// ============================================\n";

        // Generate service setter methods
        for (const auto& Prop : ParsedClass.InjectionProperties)
        {
            if (Prop.Type.find("Service") != std::string::npos)
            {
                std::string ServiceName = Prop.Type;
                // Extract service interface name (e.g., "IService*" -> "Service")
                size_t StarPos = ServiceName.find('*');
                if (StarPos != std::string::npos)
                {
                    ServiceName = ServiceName.substr(0, StarPos);
                }
                // Remove leading 'I' if present
                if (!ServiceName.empty() && ServiceName[0] == 'I')
                {
                    ServiceName = ServiceName.substr(1);
                }

                Out << "inline void Set" << ServiceName << "(" << Prop.Type << " InService)\n";
                Out << "{\n";
                Out << "    " << Prop.Name << " = InService;\n";
                Out << "}\n\n";
            }
        }
