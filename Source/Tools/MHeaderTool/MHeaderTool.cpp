...
int main(int argc, char** argv)
{
    ...
    for (const auto& Record : IR.Records)
    {
        ...
        // await — 状态机驱动函数实现（Frame 驱动，生成到 .AwaitImpl.mgenerated.cpp）
        {
            std::ostringstream AwaitImpl;
            for (const auto& F : Record.Functions)
            {
                if (!F.bIsAsync) continue;
                const MString Impl = CodeGen.EmitAwaitFuncImpl(Record.Name, F);
                if (!Impl.empty())
                {
                    AwaitImpl << Impl << "\n";
                }
            ...
            if (!ImplAll.empty())
            {
                ...
                Hdr << "// Generated by MHeaderTool (await 状态机驱动 函数实现)\n";
                Hdr << "// Source: " << Record.HeaderPath.string() << "\n\n";
                Hdr << "#include \"" << SanitizeIdentifier(Record.Name) << ".mgenerated.h\"\n";
    ...
    // 只在 IR SParsedFunction 上，legacy SFreeAsyncFunc 不携带）
    {
        ...
        for (const auto& [HeaderStr, Funcs] : AwaitFreeByHeader)
        {
            ...
            {
                if (!F.bIsAsync) continue;
                const MString Impl = CodeGen.EmitAwaitFuncImpl("Free", F);
            ...
            {
                std::ostringstream Hdr;
                Hdr << "// Generated by MHeaderTool (await 自由函数驱动实现)\n";
