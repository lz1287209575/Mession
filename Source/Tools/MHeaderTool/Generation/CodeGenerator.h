...
public:
    ...
    void GenerateClassSource(std::ostringstream& out, const SParsedClass& parsedClass) const
    {
        ...
        out << "} // namespace\n";
        // ---- Injection: 注入成员到目标类（Component Injection 系统）----
        if (!parsedClass.InjectionClass.empty())
        {
            ...
            {
                if (Prop.Type.find("Service") != MString::npos)
                {
                    MString ServiceName = Prop.Type;
                    const size_t StarPos = ServiceName.find('*');
                    if (StarPos != MString::npos) ServiceName = ServiceName.substr(0, StarPos);
                    if (!ServiceName.empty() && ServiceName[0] == 'I') ServiceName = ServiceName.substr(1);
