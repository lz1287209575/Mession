...
private:
    ...
    template<typename T>
    void Assign(const FString& Key, T&& V)
    {
        if (!Key.empty())
        {
...
private:
    ...
    static void EscapeString(const FString& In, TString& Out)
    {
        for (char C : In)
        {
            switch (C)
    ...
    void WriteString(const FString& S)
    {
        Result.push_back('\"');
...
struct MJsonValue
{
    ...
    FString StringValue;
    TMap<FString, MJsonValue> ObjectValue;
...
{
public:
    static bool Parse(const FString& Text, MJsonValue& OutValue, FString& OutError)
    {
        MJsonReader R(Text);
        R.SkipWhitespace();
...
private:
    explicit MJsonReader(const FString& InText)
        : Text(InText)
    {
    }
    ...
    bool ParseNumber(double& Out)
    {
        ...
        FString NumStr = Text.substr(Start, Index - Start);
        try
        {
            Out = std::stod(NumStr);
        }
    ...
    bool ParseObject(TMap<FString, MJsonValue>& Out)
    {
        if (Get() != '{')
        {
