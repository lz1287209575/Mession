...
namespace RpcClientCall
{
    ...
    inline bool SendClientDownlink(TSharedPtr<INetConnection> Conn, uint16 FunctionId, const TByteArray& Payload)
    {
        ...
        TByteArray Packet;
        if (!BuildClientEnvelopePacket(FunctionId, /*RequestId=*/0, Payload, Packet)) return false;
    ...
    inline size_t CallClient(uint16 FunctionId, const TByteArray& Payload, bool bBroadcast = false)
    {
        ...
        const TVector<TSharedPtr<INetConnection>> Targets =
            bBroadcast ? Resolver.ResolveBroadcast() : Resolver.ResolveCurrentContext();
        size_t Sent = 0;
        for (const auto& Conn : Targets)
        {
            if (SendClientDownlink(Conn, FunctionId, Payload)) ++Sent;
        }
        return Sent;
    }
}