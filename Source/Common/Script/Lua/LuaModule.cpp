...
MLuaModule::MLuaModule(lua_State* InL, MClass* InCls)
    : L(InL), OwningClass(InCls)
{
    lua_newtable(L);
...
{
    if (!Cls || Cls != OwningClass || !L) return;
...
void MLuaModule::RegisterFunction(MFunctionObject* Fn)
{
    ...
    lua_pushcfunction(L, +[](lua_State* L) -> int { return luaL_error(L, "LuaBindEmitter not yet wired"); });
    lua_settable(L, -3);
    lua_pop(L, 1);
}
