...
struct FScriptError
{
    ...
    FScriptError(const char* InCode, MString InMessage, MString InStack = MString())
        : Code(InCode), Message(std::move(InMessage)), StackTrace(std::move(InStack)) {}

    FScriptError(MString InCode, MString InMessage, MString InStack = MString())
        : Code(std::move(InCode)), Message(std::move(InMessage)), StackTrace(std::move(InStack)) {}
};
