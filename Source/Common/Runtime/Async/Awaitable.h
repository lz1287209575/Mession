 ...
 */
template <auto F> class TAwaitable {
    public:
    template <typename... Args> explicit TAwaitable(Args&&...) {
    }
    ...
    operator RetType() const {
        ...
        P.SetValue(TResult<ResultType, FAppError>::Err(FAppError{"await_not_wired", "TAwaitable 运行时需经 codegen 生成的状态机驱动实现"}));
        return RetType(P.GetFuture());
    }

    // AsAwaiter() 由 await 状态机 codegen 注入——类型层只提供构造与占位转换
    auto AsAwaiter() {
        static_assert(std::is_void_v<decltype(F)>, "TAwaitable::AsAwaiter() 由 await 状态机 codegen 注入");
    }
};
