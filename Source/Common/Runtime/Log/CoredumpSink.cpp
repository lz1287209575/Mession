...
void MCoredumpSink::HandleFatal(const SConfig& Config, const SLogRecord& TriggeringRecord, const TVector<SLogRecord>& TailRecords, const MString& TriggeringMessage) {
    ...
    Out << "{\"event\":\"trigger\",\"msg\":\"" << JsonEscape(TriggeringMessage).c_str() << "\",\"record\":" << RenderRecord(TriggeringRecord).c_str() << "}\n";
    ...
    Ctx.Value(Pid);
    Ctx.Key("hostname");
    Ctx.Value(MString(Host));
    Ctx.Key("status");
    Ctx.Value(Status);
    Ctx.Key("maps_excerpt");
    Ctx.Value(Maps.substr(0, std::min<size_t>(Maps.size(), 8192)));
    Ctx.EndObject();
