...
bool MTcpSink::Open()
{
    ...
    sockaddr_in Addr;
    if (!ResolveTarget(Target, Addr)) return false;
...
{
    if (Batch.empty() || OutBuffer.empty()) return;
    ...
    if (!bOpen)
    {
        ...
        sockaddr_in Addr;
        if (!ResolveTarget(Target, Addr)) return;
        SockFd = ::socket(AF_INET, SOCK_STREAM, 0);
        if (SockFd < 0)
        {
    ...
    {
        const ELogLevel Lvl = static_cast<ELogLevel>(R.Level);
        if (static_cast<int>(Lvl) < static_cast<int>(MinLevelValue)) continue;
