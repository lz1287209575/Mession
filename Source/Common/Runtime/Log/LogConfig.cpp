...
namespace {
    ...
    bool ReadString(SLex& L, MString& Out) {
        ...
        Out.clear();
        while (L.P && *L.P && *L.P != ',' && *L.P != '}' && *L.P != ']' && *L.P != ' ' && *L.P != '\t' && *L.P != '\n' && *L.P != '\r') {
            Out.push_back(*L.P++);
        }
        return !Out.empty();
    }
...
bool MLogApplyConfigFile(const MString& Path, SLogInitParams& InOutParams, TVector<SLogCategoryConfig>& OutCategories, TVector<SLogRouteConfig>& OutRoutes) {
    std::ifstream F(Path);
    if (!F.is_open())
        return false;
    ...
    while (true) {
        ...
        if (Key == "defaultLevel") {
            ...
            if (!ReadString(L, V))
                return false;
            if (V == "Trace")
                InOutParams.GlobalDefaultLevel = ELogLevel::Trace;
            else if (V == "Debug")
                InOutParams.GlobalDefaultLevel = ELogLevel::Debug;
            else if (V == "Info")
                InOutParams.GlobalDefaultLevel = ELogLevel::Info;
            else if (V == "Warn")
                InOutParams.GlobalDefaultLevel = ELogLevel::Warn;
            else if (V == "Error")
                InOutParams.GlobalDefaultLevel = ELogLevel::Error;
            else if (V == "Critical")
                InOutParams.GlobalDefaultLevel = ELogLevel::Critical;
            ...
            while (!Consume(L, ']')) {
                ...
                while (!Consume(L, '}')) {
                    ...
                    if (K == "name")
                        ...
                        if (V == "Trace")
                            Cat.Level = ELogLevel::Trace;
                        else if (V == "Debug")
                            Cat.Level = ELogLevel::Debug;
                        else if (V == "Info")
                            Cat.Level = ELogLevel::Info;
                        else if (V == "Warn")
                            Cat.Level = ELogLevel::Warn;
                        else if (V == "Error")
                            Cat.Level = ELogLevel::Error;
                        else if (V == "Critical")
                            Cat.Level = ELogLevel::Critical;
                    } else if (K == "suppressed")
                        Cat.bSuppressed = (V == "true");
                    if (!Consume(L, ','))
                        break;
                }
                if (!Cat.Name.empty())
                ...
                while (!Consume(L, '}')) {
                    ...
                    if (K == "name") {
                        ...
                        if (!ReadString(L, V))
                            return false;
                        if (V == "Trace")
                            Rt.MinLevel = ELogLevel::Trace;
                        else if (V == "Debug")
                            Rt.MinLevel = ELogLevel::Debug;
                        else if (V == "Info")
                            Rt.MinLevel = ELogLevel::Info;
                        else if (V == "Warn")
                            Rt.MinLevel = ELogLevel::Warn;
                        else if (V == "Error")
                            Rt.MinLevel = ELogLevel::Error;
                        else if (V == "Critical")
                            Rt.MinLevel = ELogLevel::Critical;
                    } else if (K == "sinks") {
                        if (!Consume(L, '['))
                            return false;
                        while (!Consume(L, ']')) {
