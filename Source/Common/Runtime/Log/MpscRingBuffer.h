...
template <typename T> class TMpscRingBuffer {
    public:
    explicit TMpscRingBuffer(size_t CapacityPow2) : CapacityMask(CapacityPow2 - 1) {
    ...
    bool TryEnqueue(const T& Record, EEvictionPolicy Policy = EEvictionPolicy::DropOldest) {
        ...
        while (true) {
            ...
            if (EnqueuePos.compare_exchange_weak(Cur, Cur + 1, std::memory_order_relaxed, std::memory_order_relaxed)) {
                break;
            }
        ...
        size_t Target = Cur + 1;
        while (CurPub < Target) {
            if (PublishedHead.compare_exchange_weak(CurPub, Target, std::memory_order_release, std::memory_order_relaxed)) {
                break;
            }
        }
        return true;
