...
class MJsonWriter {
    ...
    void BeginObject() {
        EnsureContainer(MJsonImpl::object());
    }
    void EndObject() {
    }
    void BeginArray() {
        EnsureContainer(MJsonImpl::array());
    }
    void EndArray() {
    }
    ...
    template <typename T> void Assign(const MString& Key, T&& V) {
        if (!Key.empty()) {
...
class MJsonWriter {
    ...
    private:
    enum class EScopeType : uint8 { Object, Array };
...
// 轻量 Json 读取结果类型
enum class EJsonType : uint8 { Null, Boolean, Number, String, Object, Array };
...
struct MJsonValue {
    ...
    bool IsNull() const {
        return Type == EJsonType::Null;
    }
    bool IsBool() const {
        return Type == EJsonType::Boolean;
    }
    bool IsNumber() const {
        return Type == EJsonType::Number;
    }
    bool IsString() const {
        return Type == EJsonType::String;
    }
    bool IsObject() const {
        return Type == EJsonType::Object;
    }
    bool IsArray() const {
        return Type == EJsonType::Array;
    }
};
...
class MJsonReader {
    ...
    private:
    explicit MJsonReader(const MString& InText) : Text(InText) {
    }
