...
namespace MFormat {
    ...
    template <typename... TArgs> MString Format(MStringView Fmt, const TArgs&... Args) {
    ...
    template <typename T> MString ToString(T Value) {
...
class MStringBuilder {
    ...
    template <typename... TArgs> static void AppendFormat(MStringBuilder& Builder, MStringView Fmt, TArgs&&... Args) {
        fmt::format_to(std::back_inserter(Builder.Buffer()), Fmt, std::forward<TArgs>(Args)...);
    }
...
namespace MStringUtil {
    ...
    template <typename T> inline MString ToString(T Value) {
...
namespace MStringViewUtil {
    ...
    inline bool EndsWith(MStringView View, MStringView Suffix) {
        return View.size() >= Suffix.size() && View.compare(View.size() - Suffix.size(), Suffix.size(), Suffix) == 0;
    }
...
inline bool WriteUtf8LineToWindowsConsole(const MString& Line) {
    ...
    const int WideLength = MultiByteToWideChar(CP_UTF8, 0, Line.c_str(), static_cast<int>(Line.size()), nullptr, 0);
    if (WideLength <= 0) {
        return false;
    }
    ...
    if (MultiByteToWideChar(CP_UTF8, 0, Line.c_str(), static_cast<int>(Line.size()), WideLine.data(), WideLength) <= 0) {
        return false;
    }

    WideLine += L"\n";
    DWORD CharsWritten = 0;
    return WriteConsoleW(StdoutHandle, WideLine.c_str(), static_cast<DWORD>(WideLine.size()), &CharsWritten, nullptr) != 0;
}
#endif
