...
namespace MClientCallForwarding
{
...
class MForwardedClientResponseTarget final : public IClientResponseTarget
{
public:
    MForwardedClientResponseTarget(
...
inline MFuture<TResult<FForwardedClientCallResponse, FAppError>> ExecuteForwardedClientCall(
    ...
    const FForwardedClientCallRequest& Request)
{
    ...
    if (!TargetInstance)
    {
        Promise.SetValue(MakeErrorResult<FForwardedClientCallResponse>(
            FAppError::Make("client_call_target_missing", "ForwardClientCall")));
        return Future;
    }

    const TSharedPtr<MForwardedClientResponseTarget> ResponseTarget =
        MakeShared<MForwardedClientResponseTarget>(Request.GatewayConnectionId, Promise);
