...
void MRankListActor::OnMessage(const FActorMessage& InMsg) {
    ...
    case ERankListMessage::UpdateScore: {
        ...
        State.Version++;
        LOG_DEBUG("MRankListActor: UpdateScore player=%llu score=%lld (v%llu)", static_cast<unsigned long long>(PlayerId), static_cast<long long>(NewScore), static_cast<unsigned long long>(State.Version));
        break;
    }
    case ERankListMessage::GetTopN: {
        ...
        std::sort(All.begin(), All.end(), [](const TPair<uint64, int64>& A, const TPair<uint64, int64>& B) { return A.second > B.second; });
...
bool MRankListActor::RestoreState(const TByteArray& InStateBytes) {
    ...
    if (InStateBytes.size() != 4 + Count * 16) {
        LOG_WARN("MRankListActor::RestoreState: size mismatch (got %zu, expected %zu)", InStateBytes.size(), 4 + static_cast<size_t>(Count) * 16);
        return false;
    }
