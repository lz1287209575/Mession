...
class MServiceRegistry : public MNetServerBase, public MObject, public IDisposable {
    ...
    TMap<uint32, FServiceEndpoint>                   Endpoints; // by ServerId
    TMap<EServerType, uint64>                        MonotonicSeq;
    TMap<uint64, TSharedPtr<FRegistryClientSession>> Sessions; // by ConnId
    uint64                                           NextSeq = 0;
};