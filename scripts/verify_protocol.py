...
约定：小端序；字符串为 uint16 长度 + 裸字节（无结尾 \\0）。
...
# --- SPlayerLoginResponseMessage: ConnectionId(u64), PlayerId(u64), SessionKey(u32)
def serialize_player_login_response(conn_id: int, player_id: int, session_key: int) -> bytes:
    b: list[int] = []
    write_u64_le(b, conn_id)
    write_u64_le(b, player_id)
    write_u32_le(b, session_key)
    return bytes(b)

def deserialize_player_login_response(data: bytes) -> tuple[int, int, int]:
    o = 0
    conn_id, o = read_u64_le(data, o)
    player_id, o = read_u64_le(data, o)
    session_key, o = read_u32_le(data, o)
    if o != len(data):
        raise ValueError("trailing bytes")
    return conn_id, player_id, session_key


# --- SSessionValidateRequestMessage: ConnectionId(u64), PlayerId(u64), SessionKey(u32)
...
# --- SPlayerClientSyncMessage: PlayerId(u64), DataSize(u32), Data(bytes)
def serialize_player_client_sync(player_id: int, payload: bytes) -> bytes:
    b: list[int] = []
    write_u64_le(b, player_id)
    write_u32_le(b, len(payload))
    b.extend(payload)
    return bytes(b)
...
# --- SPlayerLogoutMessage: PlayerId(u64)
...
def main() -> int:
    ...
    # Fixed blob: SPlayerLoginResponse 已知字节（小端）
    # ConnectionId=1, PlayerId=10001, SessionKey=0x01020304
    fixed = bytes([
        1, 0, 0, 0, 0, 0, 0, 0,
        0x11, 0x27, 0, 0, 0, 0, 0, 0,
        4, 3, 2, 1,
    ])
